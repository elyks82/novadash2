...
def update_agent_cli():
    ...
    try:
        novadash_user = os.getenv("NOVADASH_TEST_USER_PROD") if args.env == "PROD" else os.getenv("NOVADASH_TEST_USER_STAGE")
        ...
        for agent_file in agent_files:
            ...
            agent = Agent(env=args.env, key=agent_key, owner=ObjectId(novadash_user), **agent_data)
            agent.save(upsert_query={"key": agent.key, "owner": ObjectId(novadash_user)})
