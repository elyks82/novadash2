...
from novadash_utils import custom_print, download_file, image_to_base64
...
class UserMessage(ChatMessage):
    ...
    def _get_content(self, schema, truncate_images=False):
        ...
        if self.attachments:
            attachment_files = [
                download_file(attachment, os.path.join("/tmp/novadash_file_cache/", attachment.split("/")[-1]), overwrite=False) 
...
async def prompt_llm_and_validate(messages, system_message, provider, tools):
    ...
    while num_attempts < max_attempts:
        ...
        if 1:
            ...
            url_pattern = r'https://(?:novadash|novadashartlab-stage-(?:data|prod))\.s3\.amazonaws\.com/\S+\.(?:jpg|jpeg|png|gif|bmp|webp|mp4|mp3|wav|aiff|flac)'
