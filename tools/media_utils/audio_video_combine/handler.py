...
async def audio_video_combine(args: dict, _: str = None, env: str = None):
    ...
    video_file = novadash_utils.get_file_handler(".mp4", video_url)
    output_file = tempfile.NamedTemporaryFile(suffix=".mp4", delete=False)

    if audio_url:
        audio_file = novadash_utils.get_file_handler(".mp3", audio_url)
        audio_duration = novadash_utils.get_media_duration(audio_file)
        ...
        video_duration = novadash_utils.get_media_duration(video_file)
        cmd = [
            "ffmpeg", "-y", "-loglevel", "panic",
            "-i", video_file,
