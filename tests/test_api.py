...
dotenv.load_dotenv()
NOVADASH_ADMIN_KEY = os.getenv("NOVADASH_ADMIN_KEY")
NOVADASH_TEST_USER = os.getenv("NOVADASH_TEST_USER_PROD") if args.production else os.getenv("NOVADASH_TEST_USER_STAGE")
...
def test_api_tool(tool_name, args):
    try:
        headers = {
            "Authorization": f"Bearer {NOVADASH_ADMIN_KEY}", 
            "Content-Type": "application/json"
        }
        task = {
            "workflow": tool_name,
            "args": args,
            "user": NOVADASH_TEST_USER
        }
        # task = {
        #     "workflow": tool_name,
