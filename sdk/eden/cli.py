...
from .client import novadashClient, get_api_key
...
def main():
    ...
    run_parser = subparsers.add_parser('chat', help='Chat with Novadash')
...
def interactive_chat(args):
    os.environ["NOVADASH_API_KEY"] = get_api_key().get_secret_value()
    if not os.getenv("NOVADASH_API_KEY"):
        print("Please use `novadash login` or set NOVADASH_API_KEY environment variable")
        return
        ...
        try:
            ...
            with Progress(
                ...
                async for response in client.async_chat(message, thread_id):
                    ...
                    console.print(f"[bold green]Novadash:\t{content}[/bold green]")
